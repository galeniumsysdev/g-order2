        ...
        <table id="cart" class="table table-hover table-condensed">
          ...
          <tfoot>
              ...
              @elseif($header->currency=='USD')
                $
              @endif{{number_format($total,2)}}</strong>
              </td>
    				</tr>
    				<tr class="hidden-xs">
            ...
            <tr class="hidden-xs">
              ...
              @elseif($header->status==1 or ($header->status==0 and Auth::user()->customer_id==$header->distributor_id) )
                   <td colspan="5" class="hidden-xs text-right">Tax: </td>
              @elseif($header->status==2 and  Auth::user()->customer_id==$header->distributor_id)
                    <td colspan="6" class="hidden-xs text-right">Tax: </td>
              @else
                    <td colspan="7" class="hidden-xs text-right">Tax: </td>
              @endif
    					<td class="hidden-xs text-right"><strong id="totprice2">
                @if($header->currency=='IDR')
                Rp.
            ...
            <tr class="hidden-xs">
              ...
              @elseif($header->status==1 or ($header->status==0 and Auth::user()->customer_id==$header->distributor_id) )
                   <td colspan="5" class="hidden-xs text-right">Total: </td>
              @elseif($header->status==2 and  Auth::user()->customer_id==$header->distributor_id)
                    <td colspan="6" class="hidden-xs text-right">Total: </td>
              @else
                    <td colspan="7" class="hidden-xs text-right">Total: </td>
              @endif
    					<td class="hidden-xs text-right"><strong id="totprice2">
                @if($header->currency=='IDR')
                Rp.
